...
import { MondaiSelectModal } from "./MondaiSelectModal";
import { formatExamTitle, formatExamDescription } from "../utils/examFormatters";
...
export function ExamCard({ exam }: ExamCardProps) {
  const [showMondaiModal, setShowMondaiModal] = useState(false);
  ...
  const displayTitle = formatExamTitle(exam.title);
  const displayDesc = formatExamDescription(exam.description) || "Bộ đề thi chính thức kèm đáp án và giải thích chi tiết tiếng Việt.";
    ...
    <>
      <div className="group relative rounded-3xl border border-border/80 bg-card/60 backdrop-blur-xl p-6 transition-all duration-300 hover:border-accent/40 hover:shadow-xl hover:shadow-accent/5 flex flex-col justify-between overflow-hidden">
        ...
        <div>
          ...
          {/* Sections Preview — clickable Mondai chips */}
            ...
            <div className="mt-4 space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {exam.sections.slice(0, 4).map((sec) => {
                  const hasSectionScore = sec.userBestScore !== undefined && sec.userBestScore !== null;
                  return (
                    <button
                      key={sec.id}
                      type="button"
                      onClick={() => setShowMondaiModal(true)}
                      className="group/sec flex items-center justify-between rounded-xl bg-secondary/40 px-2.5 py-1.5 text-xs text-muted-foreground hover:bg-accent/15 hover:text-accent transition-colors cursor-pointer"
                    >
        ...
        <div className="mt-6 pt-2 flex items-center gap-2">
          ...
          {exam.sections && exam.sections.length > 0 && (
            <button
              ...
              title="Chọn Mondai cụ thể để luyện tập"
            >
      ...
      {/* Mondai Selector Modal */}
      {showMondaiModal && (
        <MondaiSelectModal
          exam={exam}
          onClose={() => setShowMondaiModal(false)}
        />
      )}
    </>
  );
